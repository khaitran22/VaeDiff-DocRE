...
class PMTEMloss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        probs1 = F.softmax(
            torch.cat([th.unsqueeze(1), logit1.unsqueeze(1)], dim=1), dim=1)
        loss1 = -(torch.log(probs1[:, 1] + 1e-30) * labels).sum(1)
        loss3 = -(((probs1 * torch.log(probs1 + 1e-30)).sum(1))
                  * labels).sum(1) / count
        ...
        probs2 = F.softmax(
            torch.cat([th.unsqueeze(1), logit2.unsqueeze(1)], dim=1), dim=1)
        loss2 = -(torch.log(probs2[:, 0] + 1e-30) * (1 - p_mask)).sum(1)
        loss4 = -(((probs2 * torch.log(probs2 + 1e-30)).sum(1))
                  * (1 - p_mask)).sum(1) / count
...
class SCLoss(nn.Module):
    ...
    def forward(self, features, labels, weights=None):
        labels = labels.long()
        mask_s = torch.any((labels.unsqueeze(1) & labels).bool(),
                           dim=-1).float().fill_diagonal_(0)

        sims = F.cosine_similarity(features.unsqueeze(
            1), features.unsqueeze(0), dim=2).div(self.tau)
