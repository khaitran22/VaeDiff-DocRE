...
rel2id = json.load(open('./docred_data/meta/rel2id.json', 'r'))
...
def official_evaluate(tmp, path):
    ...
    fact_in_train_annotated = gen_train_facts(
        os.path.join(path, "train_revised.json"), truth_dir)
...
def official_evaluate_benchmark(tmp, path, train_file, dev_file):
    '''
        Adapted from the official evaluation code
    '''
    freq_keys = set(['P17', 'P131', 'P27', 'P150', 'P175',
    ...
    submission_answer_freq = [
        x for x in submission_answer if x['r'] in freq_keys]
    submission_answer_long_tail = [
    ...
    for i in range(len(submission_answer)):
        ...
        if title not in title2vectexSet:
            print(title)
            continue
        h_sent_set = [x['sent_id']
                      for x in vertexSet[submission_answer[i]['h_idx']]]
        t_sent_set = [x['sent_id']
                      for x in vertexSet[submission_answer[i]['t_idx']]]
        if findSmallestDifference(h_sent_set, t_sent_set, len(h_sent_set), len(t_sent_set)) == 0:
    ...
    if len(submission_answer_long_tail) > 0:
        re_p_long_tail = 1.0 * correct_re_long_tail / \
            len(submission_answer_long_tail)
    else:
        re_p_long_tail = 0
    ...
    if re_p_long_tail + re_r_long_tail == 0:
        ...
        re_f1_long_tail = 2.0 * re_p_long_tail * \
            re_r_long_tail / (re_p_long_tail + re_r_long_tail)
