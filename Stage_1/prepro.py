...
docred_rel2id = json.load(open('VaeDiff-DocRE/docred_data/meta/rel2id.json', 'r'))
docred_ner2id = json.load(open('VaeDiff-DocRE/docred_data/meta/ner2id.json', 'r'))
    ...
    for sample in tqdm(data, desc="Example"):
        if len(sample['labels']) > 0:
                ...
                for t in range(len(entities)):
                    ...
                    for h_m in range(len(ent2mention[h])):
                        for t_m in range(len(ent2mention[t])):
                            mention_hts.append(
                                (ent2mention[h][h_m], ent2mention[t][t_m]))
            ...
            for ent_idx in range(len(hts)):
                ...
                temp[:len(hts2mention_hts[ent_idx])] = torch.tensor(
                    hts2mention_hts[ent_idx])
            ...
            padded_mention_mask = torch.stack(
                padded_mention_mask, dim=0).long()
