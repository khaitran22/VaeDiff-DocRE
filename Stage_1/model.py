...
class AxialTransformer_by_entity(nn.Module):
    def __init__(self, emb_size=768, dropout=0.1, num_layers=2, dim_index=-1, heads=8, num_dimensions=2, ):
        ...
        self.axial_attns = nn.ModuleList([AxialAttention(
            dim=self.emb_size, dim_index=dim_index, heads=heads, num_dimensions=num_dimensions, ) for i in range(num_layers)])
        self.ffns = nn.ModuleList(
            [nn.Linear(self.emb_size, self.emb_size) for i in range(num_layers)])
        self.lns = nn.ModuleList([nn.LayerNorm(self.emb_size)
                                 for i in range(num_layers)])
        self.attn_dropouts = nn.ModuleList(
            [nn.Dropout(dropout) for i in range(num_layers)])
...
class DocREModel_KD(nn.Module):
    def __init__(self, args, config, model, emb_size=1024, block_size=64, num_labels=-1, teacher_model=None):
        ...
        self.projection = nn.Linear(
            emb_size * block_size, config.hidden_size, bias=False)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.mse_criterion = nn.MSELoss()
        self.axial_transformer = AxialTransformer_by_entity(
            emb_size=config.hidden_size, dropout=0.0, num_layers=6, heads=8)
        self.emb_size = emb_size
    ...
    def get_hrt(self, sequence_output, attention, entity_pos, hts):
        offset = 1 if self.config.transformer_type in [
            "bert", "roberta"] else 0
    ...
    def get_hrt_by_segment(self, sequence_output, attention, entity_pos, hts, segment_span):
        ...
        for i in range(len(entity_pos)):
            ...
            for e_pos in entity_pos[i]:
                # entity_lens.append(self.ent_num_emb(torch.tensor(len(e)).to(sequence_output).long()))
                e_pos = [x for x in e_pos if (
                    x[0] >= segment_start) and (x[1] < segment_end)]
                # print(len(e_pos))
            ...
            hs = torch.index_select(
                entity_embs, 0, ht_i[:, 0]).view(s_ne, s_ne, h_size)
            ts = torch.index_select(
                entity_embs, 0, ht_i[:, 1]).view(s_ne, s_ne, h_size)
            ...
            ht_att = ht_att / (ht_att.sum(1, keepdim=True) + 1e-10)
            rs = contract(
                "ld,rl->rd", sequence_output[0], ht_att).view(s_ne, s_ne, h_size)
    ...
    def encode_by_segment(self, input_ids, attention_mask, sentid_mask, ctx_window, stride):
        ...
        if seq_len <= ctx_window:
            segment_output, segment_attn = self.encode(
            ...
            for i in range(segments + 1):
                batch_input_ids.append(
                    segment_input[i * stride: i * stride + ctx_window])
                batch_input_attn.append(
                    segment_attention[i * stride: i * stride + ctx_window])
                segment_spans.append((i * stride, i * stride + ctx_window))
            batch_input_ids = torch.stack(batch_input_ids, dim=0)
            batch_input_attn = torch.stack(batch_input_attn, dim=0)
            segment_output, segment_attn = self.encode(
                batch_input_ids,  batch_input_attn)
    ...
    def encode_by_sentence(self, input_ids, attention_mask, sentid_mask, ctx_window, stride):
        ...
        segment_output, segment_attn = self.encode(
            segment_input,  segment_attention)
    ...
    def get_logits_by_segment(self, segment_span, sequence_output, attention, entity_pos, hts):
        ...
        ne = len(entity_pos[0])
        hs_e, rs_e, ts_e, logit_mask = self.get_hrt_by_segment(
            sequence_output, attention, entity_pos, hts, segment_span)
        logit_mask = torch.tensor(logit_mask).clone().to(
        ...
        b1_e = hs_e.view(bs, ne, ne, self.emb_size //
                         self.block_size, self.block_size)
        b2_e = ts_e.view(bs, ne, ne, self.emb_size //
                         self.block_size, self.block_size)
        bl_e = (b1_e.unsqueeze(5) * b2_e.unsqueeze(4)).view(bs,
                                                            ne, ne, self.emb_size * self.block_size)

        feature = self.projection(bl_e)
        feature = self.axial_transformer(feature) + feature
        logits = self.classifier(feature).squeeze()
        self_mask = (1 - torch.diag(torch.ones((ne)))
                     ).unsqueeze(-1).to(sequence_output)
    ...
    def forward(self,
        ...
        hs_e, rs_e, ts_e, _ = self.get_hrt(
            sequence_output, attention, entity_pos, hts)

        hs_e_1 = torch.tanh(self.head_extractor(
            torch.cat([hs_e, rs_e], dim=3)))
        ts_e_1 = torch.tanh(self.tail_extractor(
            torch.cat([ts_e, rs_e], dim=3)))

        b1_e = hs_e_1.view(bs, ne, ne, self.emb_size //
                           self.block_size, self.block_size)
        b2_e = ts_e_1.view(bs, ne, ne, self.emb_size //
                           self.block_size, self.block_size)
        bl_e = (b1_e.unsqueeze(5) * b2_e.unsqueeze(4)).view(bs,
                                                            ne, ne, self.emb_size * self.block_size)
        ...
        feature = self.projection(bl_e)
        feature = self.axial_transformer(feature) + feature
        feature_scl = torch.cat([feature.clone()[
                                x, :nes[x], :nes[x], :].reshape(-1, self.config.hidden_size) for x in range(len(nes))])

        logits_c = self.classifier(feature)

        self_mask = (1 - torch.diag(torch.ones((ne)))
                     ).unsqueeze(0).unsqueeze(-1).to(sequence_output)

        logits_classifier = logits_c * self_mask

        logits_classifier = torch.cat([logits_classifier.clone()[
                                      x, :nes[x], :nes[x], :].reshape(-1, self.config.num_labels) for x in range(len(nes))])

        if labels is None:
            logits = logits_classifier.view(-1, self.config.num_labels)
            output = (self.loss_fnt.get_label(
                logits, num_labels=self.num_labels), logits)

        if labels is not None:
            labels = [torch.tensor(label) for label in labels]
            labels = torch.cat(labels, dim=0).to(device)
            loss_classifier = self.loss_fnt(
                logits_classifier.view(-1, self.config.num_labels).float(), labels.float())

            # contrastive
            idx_select = (labels[:, 0] != 1).nonzero().squeeze(
                1).to(feature_scl.device)
            feats_scl = torch.index_select(feature_scl, 0, idx_select)
            labels_scl = torch.index_select(labels, 0, idx_select)
            scl_loss = self.scl_fnt(feats_scl, labels_scl)

            output = loss_classifier + scl_loss
