...
def get_label_input_ids(data_dir, tokenizer):
    ...
    rel2id['P0'] = 0
    wikiprop_df = pd.read_csv(os.path.join(
        data_dir, 'wikidata-properties.csv'))
    prop2item = {}
    id2item = {}
    ...
    for idx in range(len(wikiprop_df)):
        ...
        if prop_id in rel2id:
            ...
            id2item[rel2id[prop_id]] = {
                'name': rel_name, 'description': rel_description, 'alias': rel_alias}
    ...
    for idx in range(len(id2item)):
        ...
        all_tokens = [tokenizer.cls_token] + name_token + \
            [tokenizer.sep_token] + description_token + [tokenizer.sep_token]
        input_ids = tokenizer.convert_tokens_to_ids(all_tokens)
        label_attn_mask = [1] * len(input_ids)
        label_features.append(
            {'input_ids': input_ids, 'attention_masks': label_attn_mask})
...
def collate_fn_kd(batch):
    ...
    output = (input_ids, input_mask, labels, entity_pos, hts, mention_pos, mention_hts, padded_mention,
              padded_mention_mask, sentid_mask, teacher_logits, entity_types, segment_spans, neg_masks)
    return output
