...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # b, _, h, talking_heads, head_scale, scale, device, has_context = *x.shape, self.heads, self.talking_heads, self.head_scale, self.scale, x.device, exists(context)
...
class AttentionLayers(nn.Module):
    ...
    def forward(
        ...
        for ind, (layer_type, (norm, block, residual_fn)) in enumerate(zip(self.layer_types, self.layers)):
            ...
            if self.scale_shift and layer_type in ['f']:
                x = residual_fn(out, residual, time_emb.unsqueeze(1))
            else:
